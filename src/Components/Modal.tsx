...
const ModalComp = React.forwardRef<HTMLDivElement, PropsType>(
  ({ toogle, setToogle }, ref) => {
              ...
              tags: note.tags.map((tag) => {
                return {
                  ...
                  value:
                    correctInp.find(
                      (inp) =>
                        inp.parentElement?.parentElement?.dataset.itemid ==
                        tag.id
                    )?.value || "",
                };
              }),
    ...
    return createPortal(
      <div ref={ref} className={`${styles.modal} ${toogle ? styles.show : ""}`}>
        <div>
          <Modal.Dialog>
            <Modal.Header onClick={setToogle} closeButton>
              <Modal.Title>
                <h3>Edit Tags</h3>
              </Modal.Title>
            </Modal.Header>
